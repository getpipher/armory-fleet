...
Test files (all NEW, `.test.mts`, node:test via tsx):
...
export async function gate(
  ...
  // exhausted: re-run once to capture the last value (matches the "last value" contract)
  const last = await thunk(feedback, attempts);
